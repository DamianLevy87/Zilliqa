...
class Lookup : public Executable {
  ...
  std::shared_timed_mutex mutable m_mutexLookupNodes;
  ...
  std::shared_timed_mutex mutable m_mutexSeedNodes;
  VectorOfNode m_seedNodes;

  std::shared_timed_mutex mutable m_mutexMultipliers;
  VectorOfNode m_multipliers;

  std::shared_timed_mutex mutable m_mutexL2lDataProviders;
 ...
 public:
  ...
  void SendMessageToRandomLookupNode(const bytes& message);
