...
void Lookup::UpdateNodes(VectorOfNode& resultNodes, unsigned int nodePort,
  ...
  VectorOfNode tmp;
  if (QUERY_DNS_FOR_SEED && QueryNodesFromDns(tmp, dns, nodePort)) {
...
void Lookup::SetLookupNodes() {
  ...
  lock(m_mutexLookupNodes, m_mutexMultipliers);
  lock_guard<shared_timed_mutex> lock1(m_mutexLookupNodes, adopt_lock);
  lock_guard<shared_timed_mutex> lock2(m_mutexMultipliers, adopt_lock);

  m_startedTxnBatchThread = false;
  m_lookupNodesOffline.clear();

  UpdateNodes(m_lookupNodes, DEFAULT_SEED_PORT, LOOKUPS_DNS, "node.lookups");
  UpdateNodes(m_multipliers, DEFAULT_SEED_PORT, MULTIPLIER_DNS,
              "node.multipliers");

  // Remove existing lookups from multiplier if there's any
  for (const auto& lookup : m_lookupNodes) {
    auto mulItr = find_if(m_multipliers.begin(), m_multipliers.end(),
                          [&lookup](const PairOfNode& node) {
                            return lookup.first == node.first;
                          });
    bool isMul = mulItr != m_multipliers.end();
    if (isMul) m_multipliers.erase(mulItr);

    if (lookup.first == m_mediator.m_selfKey.second) m_level = isMul ? 1 : 0;
  }

  for (const auto& mulNode : m_multipliers) {
    if (mulNode.first == m_mediator.m_selfKey.second) m_level = 1;
  }

  // Add multipliers to lookups
  m_lookupNodes.insert(m_lookupNodes.end(), m_multipliers.begin(),
                       m_multipliers.end());
...
VectorOfNode Lookup::GetLookupNodesWithoutMultipliers() const {
  ...
  shared_lock<shared_timed_mutex> lock1(m_mutexLookupNodes, defer_lock);
  shared_lock<shared_timed_mutex> lock2(m_mutexMultipliers, defer_lock);
  lock(m_mutexLookupNodes, m_mutexMultipliers);

  if (m_lookupNodes.size() == 0) return m_lookupNodes;
...
void Lookup::SendMessageToLookupNodes(const bytes& message) const {
  ...
  if (QUERY_DNS_FOR_SEED) {
    if (!ObtainIpListFromDns(ipList, LOOKUPS_DNS) ||
        !ObtainIpListFromDns(ipList, MULTIPLIER_DNS)) {
      ipList.clear();
    }
  }
  ...
  if (ipList.size() == 0) {
    LOG_GENERAL(
        INFO, "Unable to get lookup nodes info from DNS, using current list.");
    auto lookups = GetLookupNodes();
    // Failed to get anything from DNS side, use what we currently have
    // (Previous DNS query, or static)
...
void Lookup::SendMessageToLookupNodesSerial(const bytes& message) const {
  ...
  if (ipList.empty()) {
    LOG_GENERAL(WARNING, "Unable to obtain IPs from DNS "
                             << LOOKUPS_DNS << ", using current list.");
...
void Lookup::SendMessageToRandomLookupNode(const bytes& message) {
  ...
  vector<uint128_t> lookupIpsFromDns;
  if (QUERY_DNS_FOR_SEED) {
    if (ObtainIpListFromDns(lookupIpsFromDns, LOOKUPS_DNS)) {
      lookupIpsFromDns.erase(
          std::remove(lookupIpsFromDns.begin(), lookupIpsFromDns.end(), selfIp),
          lookupIpsFromDns.end());
    }
  }

  if (lookupIpsFromDns.empty()) {
    // Fallback to previous dns query list or static config
    LOG_GENERAL(WARNING, "Unable to obtain IPs from DNS "
                             << LOOKUPS_DNS << ", using current list.");
    ...
    vector<uint128_t> choices;
    std::sort(lookupIpsFromDns.begin(), lookupIpsFromDns.end());
    std::sort(lookupIpsFromLocal.begin(), lookupIpsFromLocal.end());
    std::set_intersection(lookupIpsFromDns.begin(), lookupIpsFromDns.end(),
                          lookupIpsFromLocal.begin(), lookupIpsFromLocal.end(),
                          std::back_inserter(choices));

    if (choices.empty()) {
      // For some reason, we do not have any lookups from our own list in the
      // DNS Choose 1 from DNS
      int index = RandomGenerator::GetRandomInt(lookupIpsFromDns.size());
      auto ip = lookupIpsFromDns[index];
      randomPeer = Peer{ip, DEFAULT_SEED_PORT};

      bytes pubkeyBytes;
      auto ipStr = IPConverter::ToStrFromNumericalIP(ip);
      if (!ObtainPubKeyFromUrl(pubkeyBytes, ipStr, LOOKUPS_DNS)) {
      ...
      PubKey pubKey{pubkeyBytes, 0};
      {
        ...
        // Add back to local list for verification later on
        lock_guard<shared_timed_mutex> lock(m_mutexLookupNodes);
        m_lookupNodes.emplace_back(pubKey, randomPeer);
        m_lookupNodesStatic.emplace_back(pubKey, randomPeer);
      }

    } else {
      int index = RandomGenerator::GetRandomInt(choices.size());
      randomPeer = Peer{choices[index], DEFAULT_SEED_PORT};
    }
  }
...
void Lookup::SendMessageToSeedNodes(const bytes& message) const {
  ...
  if (QUERY_DNS_FOR_SEED) {
    ObtainIpListFromDns(seedIps, UPPER_SEED_DNS);
  }

  if (seedIps.empty()) {
    LOG_GENERAL(WARNING, "Unable to obtain IPs from DNS "
                             << UPPER_SEED_DNS << ", using current list.");
